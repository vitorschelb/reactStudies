import { useEffect, useContext } from "react"; //1,1.Determinar o useState
...
//16.1 REFATORADO: Movido para books.js
//16.1 Agora que todas as funções abaixo foram movidas para book.js, não precisaremos comunica-las no JSX como props, e sim conecta-las aos Provider!
//16.2 Vamos importar no topo da tela o useContext do react, e BooksContext. Perceba que não é importado o Provider, pois o Provider contem as funções que estão dentro de BooksContext. Ele é importado diretamente quando é utilizado em seus componentes especificos.
//16.3 Agora vamos chamar apenas a função Fetch que nos interessa em app, e de tudo que o BookContext nos oferece, selecionamos a função desejada. "const {fetchBooks} = useContext(BooksContext)""
//16.4 Como não estamos usando mais uma função de useState, pode deletar do topo, junto com o axios, esse passar em books.js.
...
//   // 12 APOS INSTALAR O AXIOS E CONFIGURAR O JSON SERVER, FAZER A FUNÇÃO DE POST.
